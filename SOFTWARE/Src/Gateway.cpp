...
std::string Gateway::Command(const std::string &COM,bool save) {
    ...
    if (command == "set") {
        ...
        this->save_config();
        NVIC_SystemReset();
    } else if(command == "output"){//OUTPUT 1 1
        if (this->OUT == nullptr){
            return "output not inital\r\n";
        } else {
            if (name == "1") {
                if ((value == "1") || (value == "true")) {
                    this->OUT->set(1);
                    return "output 1 set 1 success\r\n";
                } else if ((value == "0") || (value == "false")) {
                    this->OUT->set(0);
                    return "output 1 set 0 success\r\n";
                } else {
                    return "output 1 set error\r\n";
                }
            } else {
                return "output name error\r\n";
            }
        }
        ...
        return "Available commands:\r\n"
               ...
               "  output <1> <1> - set output 1 to 1\r\n"
               "  lora send <0012A920> <0> <1> - set lora id 0012A920 port 0 set 1\r\n"
               "  485 send <00> <0> <1> - set 485 id 00 port 0 set 1\r\n";
    }else{
...
void Gateway::link_OUT(_OutPut_ *OUT_t) {
    this->OUT=OUT_t;
}
