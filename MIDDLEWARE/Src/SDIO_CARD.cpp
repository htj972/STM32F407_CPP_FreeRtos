...
uint8_t SDIO_CARD::CardType=SDIO_STD_CAPACITY_SD_CARD_V1_1;		//SD卡类型（默认为1.x卡）
uint32_t SDIO_CARD::CSD_Tab[4];
uint32_t SDIO_CARD::CID_Tab[4];
uint32_t SDIO_CARD::RCA=0;					//SD卡CSD,CID以及相对地址(RCA)数据
uint8_t SDIO_CARD::DeviceMode=SD_DMA_MODE;		   				//工作模式,注意,工作模式必须通过SD_SetDeviceMode,后才算数.这里只是定义一个默认的模式(SD_DMA_MODE)
uint8_t SDIO_CARD::StopCondition=0; 								//是否发送停止传输标志位,DMA多块读写的时候用到
SD_Error SDIO_CARD::TransferError=SD_OK;					//数据传输错误标志,DMA读写时使用
uint8_t SDIO_CARD::TransferEnd=0;								//传输结束标志,DMA读写时使用
...
SD_Error SDIO_CARD::init()
{
    ...
    errorstatus=PowerON();			//SD卡上电
    if(errorstatus==SD_OK)errorstatus=InitializeCards();			//初始化SD卡
    if(errorstatus==SD_OK)errorstatus=GetCardInfo(&SDCardInfo);	//获取卡信息
    if(errorstatus==SD_OK)errorstatus=SelectDeselect((uint32_t)(SDCardInfo.RCA<<16));//选中SD卡
    if(errorstatus==SD_OK)errorstatus=EnableWideBusOperation(SDIO_BusWide_4b);	//4位宽度,如果是MMC卡,则不能用4位模式
    if((errorstatus==SD_OK)||(SDIO_MULTIMEDIA_CARD==CardType))
    {
        ...
        errorstatus=SetDeviceMode(SD_DMA_MODE);	//设置为DMA模式
//        errorstatus=SetDeviceMode(SD_POLLING_MODE);//设置为查询模式
    }
...
SD_Error SDIO_CARD::WriteBlock(uint8_t *buf,long long addr,  u16 blksize)
{
    ...
    if(buf==nullptr)return SD_INVALID_PARAMETER;//参数错误
    ...
    }else if(DeviceMode==SD_DMA_MODE)
    {
        ...
        DMA_Config((uint32_t*)buf,blksize,DMA_DIR_MemoryToPeripheral);				//SDIO DMA配置
        SDIO->DCTRL|=1<<3;								//SDIO DMA使能.
        while(((DMA2->LISR&(1<<27))==RESET)&&timeout)timeout--;//等待传输完成
        if(timeout==0)
        {
...
SD_Error SDIO_CARD::WriteMultiBlocks(uint8_t *buf,long long addr,u16 blksize,uint32_t nblks)
{
    ...
    if(nblks>1)
    {
        ...
        }else if(DeviceMode==SD_DMA_MODE)
        {
            ...
            TransferEnd=0;				//传输结束标置位，在中断服务置1
            SDIO->MASK|=(1<<1)|(1<<3)|(1<<8)|(1<<4)|(1<<9);	//配置产生数据接收完成中断
            DMA_Config((uint32_t*)buf,nblks*blksize,DMA_DIR_MemoryToPeripheral);		//SDIO DMA配置
            SDIO->DCTRL|=1<<3;								//SDIO DMA使能.
            timeout=SDIO_DATATIMEOUT;
            while(((DMA2->LISR&(1<<27))==RESET)&&timeout)timeout--;//等待传输完成
            if(timeout==0)	 								//超时
...
uint8_t SDIO_CARD::ReadDisk(uint8_t* buf,uint32_t sector,uint8_t cnt)
{
    ...
    if((uint32_t)buf%4!=0)
    {
        for(n=0;n<cnt;n++)
        {
            sta=ReadBlock(SDIO_DATA_BUFFER,lsector+512*n,512);//单个sector的读操作
            memcpy(buf,SDIO_DATA_BUFFER,512);
            buf+=512;
        }
    }else
    {
        if(cnt==1)sta=ReadBlock(buf,lsector,512);    	//单个sector的读操作
        else sta=ReadMultiBlocks(buf,lsector,512,cnt);//多个sector
    }
    return sta;
}
//写SD卡
...
uint8_t SDIO_CARD::WriteDisk(uint8_t* buf,uint32_t sector,uint8_t cnt)
{
    ...
    if((uint32_t)buf%4!=0)
    {
        for(n=0;n<cnt;n++)
        {
            ...
            sta=WriteBlock(SDIO_DATA_BUFFER,lsector+512*n,512);//单个sector的写操作
            buf+=512;
        }
    }else
    {
        if(cnt==1)sta=WriteBlock(buf,lsector,512);    	//单个sector的写操作
        else sta=WriteMultiBlocks(buf,lsector,512,cnt);	//多个sector
    }
    return sta;
}
...
//        f_mount(fs[0],"0:",1);
//        time_i=WAIT_TIME;
//        while(exf_getfree((uint8_t )"0:",&total,&free)&&time_i)	//得到SD卡的总容量和剩余容量
//        {
//            time_i--;
