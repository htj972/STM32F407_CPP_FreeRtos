...
__attribute__((aligned(4))) ETH_DMADESCTypeDef *DMARxDscrTab;	//以太网DMA接收描述符数据结构体指针
__attribute__((aligned(4))) ETH_DMADESCTypeDef *DMATxDscrTab;	//以太网DMA发送描述符数据结构体指针
__attribute__((aligned(4))) uint8_t *Rx_Buff; 					//以太网底层驱动接收buffers指针
__attribute__((aligned(4))) uint8_t *Tx_Buff; 					//以太网底层驱动发送buffers指针
...
uint8_t LAN8720_Init(void)
{
    ...
    /*网络引脚设置 RMII接口
      ETH_MDIO -------------------------> PA2      1
      ETH_MDC --------------------------> PC1      1
      ETH_RMII_REF_CLK------------------> PA1      1
      ETH_RMII_CRS_DV ------------------> PA7      1
      ETH_RMII_RXD0 --------------------> PC4      1
      ETH_RMII_RXD1 --------------------> PC5      1
      ETH_RMII_TX_EN -------------------> PB11
      ETH_RMII_TXD0 --------------------> PB12
    ...
    //配置PD3为推完输出
    GPIO_InitStructure.GPIO_Pin = LAN8720_RST_GPIO;
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;
    GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;	//推完输出
    GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_NOPULL ;
    GPIO_Init(LAN8720_RST_PORT, &GPIO_InitStructure);

    GPIO_ResetBits(LAN8720_RST_PORT,LAN8720_RST_GPIO);           //硬件复位LAN8720
    delay_ms(50);
    GPIO_SetBits(LAN8720_RST_PORT,LAN8720_RST_GPIO);			 	//复位结束
...
FrameTypeDef ETH_Rx_Packet(void)
{
    ...
    {
        frame_length=((DMARxDescToGet->Status&ETH_DMARxDesc_FL)>>ETH_DMARxDesc_FrameLengthShift)-4;//得到接收包帧长度(不包含4字节CRC)
        frame.buffer = DMARxDescToGet->Buffer1Addr;//得到包数据所在的位置
    }else frame_length=ETH_ERROR;//错误
